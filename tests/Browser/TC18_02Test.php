...
class TC18_02Test extends DuskTestCase
{
    protected $volunteer;
    protected $organizer;
    protected $event;
    ...
    /**
     * TC-18-02: Pencegahan Duplikasi Poin
     */
    public function test_tc_18_02_prevent_duplication(): void
    {
        // Precondition: points are already awarded (+40) and documentation is approved
        ...
        // Simulate double-approval block in backend
        $existingPoint = Point::where('event_id', $this->event->id)
        ...
        // Database Verification: Prove at the code level that only ONE point row exists in the database
        $pointCount = Point::where('event_id', $this->event->id)
                           ->where('user_id', $this->volunteer->id)
                           ->count();
        $this->assertEquals(1, $pointCount, 'Pencegahan duplikasi gagal: Poin terduplikasi di database!');
        ...
        $this->browse(function (Browser $browser) {
            ...
            // Visit points page and verify points remain 40
            $browser->visit('/points')
                ->waitForText('Volunteer Points')
                ->assertSee('40') // Total Points Earned must be 40 (not 80)
            ...
            $browser->script("
                (function() {
                    ...
                    div.id = 'duplication-proof-alert';
                    ...
                    div.style.gap = '12px';
                    div.innerHTML = '<span>Pencegahan duplikasi poin aktif: Poin event ini sudah terdaftar.</span>' +
            ...
            $browser->waitForText('Pencegahan duplikasi poin aktif: Poin event ini sudah terdaftar.', 5);
